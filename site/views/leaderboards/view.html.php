 ...
 */
class BootcampleaderboardsViewTeams extends JView
{
	// Overwriting JView display method
	function display($tpl = null) 
	{
		$templateName = JRequest::getVar( 'teamlisttemplate', '', 'default', 'string' );
		
		$model = $this->getModel();
		$state = $this->get('State');	
		$additionalfields = $model->getListFields();
		$teams = $model->getTeamList();
		
		foreach ($teams as $team)
		{
			$members = $model->getMembers($team->id);
	...
	protected function _prepareDocument()
	{
		...
		if($menu)
		{
			...
			$this->params->def('page_heading', JText::_('COM_BOOTCAMPLEADERBOARDS_LEADERBOARDS_DEFAULT_PAGE_TITLE'));
		}
